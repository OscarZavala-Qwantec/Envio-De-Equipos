...
document.addEventListener("submit",async e=>{
  ...
  if(e.target.id==="sale-form"){
    const clientId=String(f.get("clientId")),client=clientById(clientId),currentSeries=clientSeries(client),sales=clientSales(client),raw=String(f.get("series")||"");
    const saleSeries=[...new Map(raw.split(/[\n,;]+/).map(value=>value.trim()).filter(Boolean).map(value=>[normalize(value),value])).values()];
    if(!saleSeries.length)return alert("Agrega por lo menos una serie para la nueva venta.");
    const repeated=saleSeries.filter(value=>currentSeries.some(existing=>normalize(existing)===normalize(value)));
    if(repeated.length)return alert(`Estas series ya están registradas: ${repeated.join(", ")}. Una nueva venta debe incluir series nuevas.`);
    const sale={id:crypto.randomUUID(),fecha:String(f.get("fecha")),detalle:String(f.get("detalle")||"").trim(),estadoPago:String(f.get("estadoPago"))==="pagado"?"pagado":"pendiente",series:saleSeries,creadoEn:new Date().toISOString()};
    try{setCloudStatus("Guardando venta...");await updateDoc(doc(db,"envioClientes",clientId),{series:[...currentSeries,...saleSeries],ventas:[...sales,sale],actualizadoEn:serverTimestamp()});close();setTimeout(()=>openClient(clientId),250);}catch(error){firebaseError(error);}return;
  }
...
function openSaleForm(clientId){const c=clientById(clientId);if(!c)return;modal(`Nueva venta · ${c.razon}`,`<form class="form" id="sale-form"><input type="hidden" name="clientId" value="${c.id}"><div class="client-summary"><b>${esc(c.razon)}</b><span>RUC ${esc(c.ruc)} · La venta debe contener series nuevas.</span></div><label>Fecha de venta<input name="fecha" type="date" value="${today()}" required></label><label>Descripción de la venta<input name="detalle" placeholder="Ej. 2 equipos biométricos (opcional)"></label><label>Series vendidas<textarea name="series" rows="5" required placeholder="Una serie por línea&#10;EQ-000101&#10;EQ-000102"></textarea><small>Puedes separar varias series con saltos de línea, comas o punto y coma.</small></label><label>Estado del pago<select name="estadoPago" required><option value="pendiente">Pendiente de pago</option><option value="pagado">Pagado</option></select></label><button>Guardar nueva venta</button></form>`);}
