...
document.addEventListener("submit",async e=>{
  ...
  if(e.target.id==="sale-form"){
    const clientId=String(f.get("clientId")),client=clientById(clientId),sales=clientSales(client);
    const sale={id:crypto.randomUUID(),fecha:String(f.get("fecha")),detalle:String(f.get("detalle")||"").trim(),estadoPago:String(f.get("estadoPago"))==="pagado"?"pagado":"pendiente",series:[],creadoEn:new Date().toISOString()};
    try{setCloudStatus("Guardando venta...");await updateDoc(doc(db,"envioClientes",clientId),{ventas:[...sales,sale],actualizadoEn:serverTimestamp()});close();setTimeout(()=>openClient(clientId),250);}catch(error){firebaseError(error);}return;
  }
...
function openSaleForm(clientId){const c=clientById(clientId);if(!c)return;modal(`Nueva venta · ${c.razon}`,`<form class="form" id="sale-form"><input type="hidden" name="clientId" value="${c.id}"><div class="client-summary"><b>${esc(c.razon)}</b><span>RUC ${esc(c.ruc)} · Primero registra la venta; las series se asignan después del pago.</span></div><label>Fecha de venta<input name="fecha" type="date" value="${today()}" required></label><label>Descripción de la venta<input name="detalle" placeholder="Ej. 2 equipos biométricos (opcional)"></label><label>Estado del pago<select name="estadoPago" required><option value="pendiente">Pendiente de pago</option><option value="pagado">Pagado</option></select></label><button>Guardar nueva venta</button></form>`);}
