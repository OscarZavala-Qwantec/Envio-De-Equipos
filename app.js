...
function openClient(id){const c=clientById(id);if(!c)return;const list=clientShipments(id),series=clientSeries(c),sales=clientSales(c).slice().sort((a,b)=>String(b.fecha||"").localeCompare(String(a.fecha||""))),sold=new Set(sales.flatMap(sale=>sale.series||[]).map(normalize)),unassigned=series.filter(value=>!sold.has(normalize(value))),pending=sales.filter(sale=>sale.estadoPago!=="pagado").length;modal(c.razon,`<div class="client-header"><div><span class="label">RUC</span><b>${esc(c.ruc)}</b></div><div><span class="label">CONTACTO</span><b>${esc(c.contacto)}</b></div><div class="client-actions"><button class="whatsapp-btn" data-action="whatsapp" data-id="${c.id}">${whatsappIcon} WhatsApp</button><button class="primary dark" data-action="new-shipment" data-id="${c.id}">+ Agregar envío</button></div></div><section class="sales-section"><div class="sales-heading"><div><span class="equipment-kicker">CONTROL COMERCIAL</span><h3>Ventas de equipos</h3><p>Registra la venta, confirma el pago y luego asigna sus series.</p></div><div class="sales-heading-actions"><span class="pending-total">${pending} pendientes</span><button class="primary dark" data-action="new-sale" data-id="${c.id}">+ Nueva venta</button></div></div><div class="sales-list">${sales.map(sale=>saleRow(sale,c)).join("")||'<div class="sales-empty">Todavía no hay ventas registradas. Las series existentes continúan debajo sin una venta asociada.</div>'}</div></section><section class="equipment-section"><div class="equipment-heading"><div><span class="equipment-kicker">INVENTARIO DEL CLIENTE</span><h3>Series sin venta asociada</h3><p>Aquí permanecen las series anteriores, manuales o importadas que no pertenecen a una venta.</p></div><span class="series-total">${unassigned.length}</span></div><form id="series-form" class="series-inline"><input type="hidden" name="clientId" value="${c.id}"><input name="series" placeholder="Agregar serie sin venta" autocomplete="off" required><button>+ Agregar serie</button></form><div class="series-chips">${unassigned.map(value=>`<span class="series-chip"><b>${esc(value)}</b><button type="button" title="Quitar serie" data-action="delete-client-series" data-id="${c.id}" data-series="${esc(value)}">×</button></span>`).join("")||'<span class="series-empty">No hay series sin venta asociada.</span>'}</div><div class="inventory-total">Total general de series: <b>${series.length}</b></div></section><h3 class="subheading">Historial de envíos</h3><div class="shipment-list">${list.map(s=>shipmentRow(s,c,true)).join("")||'<div class="empty">Todavía no hay envíos registrados para este cliente.</div>'}</div>`,true);}
function saleRow(sale,c){const paid=sale.estadoPago==="pagado",series=Array.isArray(sale.series)?sale.series:[];return `<article class="sale-row"><div class="sale-main"><div><span class="label">FECHA DE VENTA</span><b>${formatDate(sale.fecha)}</b></div><div><span class="label">DETALLE</span><b>${esc(sale.detalle)||"Venta de equipos"}</b></div><span class="payment-badge ${paid?"paid":"pending"}">${paid?"Pagado":"Pendiente"}</span></div><div class="sale-series"><span class="label">SERIES DE ESTA VENTA</span><div class="series-chips">${series.map(value=>`<span class="series-chip sale"><b>${esc(value)}</b></span>`).join("")||'<span class="series-empty">Sin series asignadas</span>'}</div></div><div class="sale-controls"><button class="edit-sale-btn" data-action="edit-sale" data-client="${c.id}" data-sale="${esc(sale.id)}">Editar venta</button><button class="payment-toggle ${paid?"paid":""}" data-action="toggle-sale-payment" data-client="${c.id}" data-sale="${esc(sale.id)}">${paid?"Marcar pendiente":"Marcar como pagado"}</button>${paid?`<button class="assign-series-btn" data-action="assign-sale-series" data-client="${c.id}" data-sale="${esc(sale.id)}">+ Asignar series</button>`:'<span class="sale-waiting">Confirma el pago para asignar series</span>'}</div></article>`;}
