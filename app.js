...
document.addEventListener("submit",async e=>{
  ...
  if(e.target.id==="client-form"){
    ...
    try{setCloudStatus("Guardando...");const result=await addDoc(clientsRef,{ruc,razon:String(f.get("razon")).trim(),contacto:String(f.get("contacto")).trim(),series:[],creadoEn:serverTimestamp()});close();setTimeout(()=>openClient(result.id),250);}catch(error){firebaseError(error);}
  }
...
function openClient(id){const c=clientById(id);if(!c)return;const list=clientShipments(id),series=clientSeries(c);modal(c.razon,`<div class="client-header"><div><span class="label">RUC</span><b>${esc(c.ruc)}</b></div><div><span class="label">CONTACTO</span><b>${esc(c.contacto)}</b></div><div class="client-actions"><button class="whatsapp-btn" data-action="whatsapp" data-id="${c.id}">${whatsappIcon} WhatsApp</button><button class="primary dark" data-action="new-shipment" data-id="${c.id}">+ Agregar envío</button></div></div><section class="equipment-section"><div class="equipment-heading"><div><span class="equipment-kicker">INVENTARIO DEL CLIENTE</span><h3>Series de equipos</h3><p>Equipos que el cliente tiene en general; son independientes del historial de envíos.</p></div><span class="series-total">${series.length}</span></div><form id="series-form" class="series-inline"><input type="hidden" name="clientId" value="${c.id}"><input name="series" placeholder="Escribe una serie de equipo" autocomplete="off" required><button>+ Agregar serie</button></form><div class="series-chips">${series.map(value=>`<span class="series-chip"><b>${esc(value)}</b><button type="button" title="Quitar serie" data-action="delete-client-series" data-id="${c.id}" data-series="${esc(value)}">×</button></span>`).join("")||'<span class="series-empty">Aún no hay series generales registradas.</span>'}</div></section><h3 class="subheading">Historial de envíos</h3><div class="shipment-list">${list.map(s=>shipmentRow(s,c,true)).join("")||'<div class="empty">Todavía no hay envíos registrados para este cliente.</div>'}</div>`,true);}
