...
document.addEventListener("submit",async e=>{
  ...
  if(e.target.id==="shipment-form"){
    ...
    if(file&&file.size){
      ...
      try{setCloudStatus("Subiendo archivo...");const safeName=file.name.replace(/[^a-zA-Z0-9._-]/g,"_");const path=`${f.get("clientId")}/${Date.now()}-${crypto.randomUUID()}-${safeName}`;const {error}=await supabase.storage.from("workera-envios").upload(path,file,{contentType:file.type,upsert:false});if(error)throw error;const {data}=supabase.storage.from("workera-envios").getPublicUrl(path);documento={nombre:file.name,tipo:file.type,url:data.publicUrl,path};}catch(error){firebaseError(error);return;}
    }
    try{setCloudStatus("Guardando...");await addDoc(shipmentsRef,{clientId:String(f.get("clientId")),fecha:f.get("fecha"),serie:String(f.get("serie")||"").trim(),tracking:String(f.get("tracking")||"").trim(),nota:String(f.get("nota")||"").trim(),documento,creadoEn:serverTimestamp()});close();}catch(error){if(documento?.path)await supabase.storage.from("workera-envios").remove([documento.path]);firebaseError(error);}
  }
});
